...
interface DataContextType {
  ...
  whatIfResults: WhatIfAnalysisResult[];
  addWhatIfResult: (result: WhatIfAnalysisResult) => void;
  getWhatIfResultsByOperation: (operationType: OperationType) => WhatIfAnalysisResult[];
  getLatestWhatIfResult: (operationType: OperationType) => WhatIfAnalysisResult | undefined;
  clearWhatIfResults: () => void;
}
...
export function DataProvider({ children }: DataProviderProps) {
  ...
  const [whatIfResults, setWhatIfResults] = useState<WhatIfAnalysisResult[]>([]);
  const [isInitialized, setIsInitialized] = useState(false);

  // Load persisted datasets and what-if results on mount
  useEffect(() => {
    loadPersistedDatasets();
    loadPersistedWhatIfResults();
  }, []);
  ...
  // What-If Analysis Results persistence methods
  const loadPersistedWhatIfResults = () => {
    try {
      const storedResults = localStorage.getItem('rto_whatif_results');
      if (storedResults) {
        const parsedResults = JSON.parse(storedResults);
        // Convert date strings back to Date objects
        const results = parsedResults.map((result: any) => ({
          ...result,
      ...
      console.error('Failed to load What-If results:', error);
    }
  };

  const saveWhatIfResults = (results: WhatIfAnalysisResult[]) => {
    try {
      // Keep only the last 50 results to manage storage
      const limitedResults = results.slice(-50);
      localStorage.setItem('rto_whatif_results', JSON.stringify(limitedResults));
    } catch (error) {
      console.error('Failed to save What-If results:', error);
      // If storage is full, keep only the last 10 results
      try {
        const minimalResults = results.slice(-10);
        localStorage.setItem('rto_whatif_results', JSON.stringify(minimalResults));
        toast.warning('What-If results storage limited due to space constraints');
      } catch (finalError) {
        toast.error('Unable to save What-If analysis results');
      }
    }
  };

  // Save What-If results whenever they change
  useEffect(() => {
    if (isInitialized && whatIfResults.length > 0) {
      saveWhatIfResults(whatIfResults);
    }
  }, [whatIfResults, isInitialized]);
  ...
  // What-If Analysis methods
  const addWhatIfResult = (result: WhatIfAnalysisResult) => {
    setWhatIfResults(prev => [...prev, result]);
    toast.success('What-If analysis result saved');
  };

  const getWhatIfResultsByOperation = (operationType: OperationType) => {
    return whatIfResults.filter(r => r.operationType === operationType);
  };

  const getLatestWhatIfResult = (operationType: OperationType) => {
    const results = getWhatIfResultsByOperation(operationType);
    return results.length > 0 ? results[results.length - 1] : undefined;
  };

  const clearWhatIfResults = () => {
    setWhatIfResults([]);
    try {
      localStorage.removeItem('rto_whatif_results');
    } catch (error) {
      console.error('Failed to clear What-If results storage:', error);
    }
    toast.info('All What-If analysis results cleared');
  };
  ...
  return (
    <DataContext.Provider value={{
      ...
      whatIfResults,
      addWhatIfResult,
      getWhatIfResultsByOperation,
      getLatestWhatIfResult,
      clearWhatIfResults
    }}>
      {children}
