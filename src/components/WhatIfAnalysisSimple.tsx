...
import { useDataContext, DatasetInfo, WhatIfAnalysisResult } from './DataContext';
...
export function WhatIfAnalysisSimple({ onNavigate }: WhatIfAnalysisSimpleProps) {
  ...
  const { datasets, getDataset, addWhatIfResult } = useDataContext();
  ...
  const handleRunSimulation = async () => {
    ...
    try {
      ...
      // Create and save What-If analysis result
      const whatIfResult: WhatIfAnalysisResult = {
        id: `whatif-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        timestamp: new Date(),
        operationType: currentDataset.operationType,
        ...
        projectedChange: results.projectedChange,
        insights: results.insights,
        recommendations: results.recommendations
      };

      // Save to context for use by CustomizeDashboard
      addWhatIfResult(whatIfResult);
